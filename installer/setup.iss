...
WelcomeLabel2=This installer fixes known issues in Duo Manager 1.5.6:%n%n  1. Resolution locked at 640x480%n  2. Broken web management interface%n  3. Virtual audio sink default overwritten on reconnect%n%nPrerequisite: Duo Manager 1.5.6 must be installed.%n%nClick Next to continue.
...
; Web UI assets (management interface)
Source: "..\bundled\apollo\web\pin.html";     DestDir: "{tmp}\web"; Flags: deleteafterinstall
Source: "..\bundled\apollo\web\login.html";   DestDir: "{tmp}\web"; Flags: deleteafterinstall
Source: "..\bundled\apollo\web\welcome.html"; DestDir: "{tmp}\web"; Flags: deleteafterinstall
Source: "..\bundled\apollo\web\assets\*";     DestDir: "{tmp}\web\assets"; Flags: recursesubdirs deleteafterinstall
...
begin
  ...
  // ----------------------------------------------------------
  // Fix 2: Web assets (management interface)
  // ----------------------------------------------------------
  ...
  // ----------------------------------------------------------
  // Fix 3: Games_apps.json (only if it doesn't exist - preserves user customizations)
  // ----------------------------------------------------------
  ...
  // ----------------------------------------------------------
  // Fix 4: Sunshine conf - enable debug logging to capture Moonlight resolution
  // Scans config/*.conf, skips duo_wrapper.conf, patches the most recently
  // modified conf that contains min_log_level or log_path.
  // ----------------------------------------------------------
  Exec('powershell.exe',
    '-NoProfile -Command "' +
    '$dir = ''' + DuoDir + '\config''; ' +
  ...
  // ----------------------------------------------------------
  // Fix 5: Duo.exe binary patch - zero out hardcoded virtual_sink config entry
  ...
  Exec('powershell.exe',
    ...
    '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  // Fix 5 is optional — if the pattern is not found (different Duo version),
  // skip silently. The DuoRdpWrapper resolution fix still works without this patch.
